...
class Rnn extends AbstractCodegenerator
{
    ...
    public function generateTrainingScript(TrainingPathGenerator $pathGenerator): string
    {
        ...
        $innerLines[] = "features_train, features_test, target_train, target_test = train_test_split(";
        $innerLines[] = "features, target, test_size=0.2, random_state=1337";
        $innerLines[] = ")";

        $innerLines[] = "# Umwandlung der Daten in 3D-Format für LSTM und GRU";
//        $innerLines[] = "features_train = np.reshape(features_train, (features_train.shape[0], 1, features_train.shape[1]))";
        $innerLines[] = "features_train = features_train.values[:, np.newaxis, :]";
//        $innerLines[] = "features_test = np.reshape(features_test, (features_test.shape[0], 1, features_test.shape[1]))";
        $innerLines[] = "features_test = features_test.values[:, np.newaxis, :]";

//        $innerLines[] = "features_train, features_test, target_train, target_test = train_test_split(features, target, test_size=0.15, random_state=42)";
//        $innerLines[] = "features_train, x_val, target_train, y_val = train_test_split(features_train, target_train, test_size=0.33333333333333, random_state=42)";
...
//                $this->getEarlystopMonitor($hyperparameter['costFunction']),
